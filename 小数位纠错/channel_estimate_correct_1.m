...
a_correct_data=zeros(1,m);
b_correct_data=zeros(1,m);
e_correct_data=zeros(1,m);
...
for i=1:m
    ...
    a_correct_data(i)=calculate_correct_data_10(a_interval(i),ha_abs_array(i));
    b_correct_data(i)=calculate_correct_data_10(b_interval(i),hb_abs_array(i)); 
    e_correct_data(i)=calculate_correct_data_10(e_interval(i),ge_abs_array(i)); 

    if a_correct_data(i)<0.5 && b_correct_data(i)>=0.5
        if b_correct_data(i)>a_correct_data(i)+0.5 && b_interval(i)<10
            b_interval(i)=b_interval(i)+1;
        end
    end
    if a_correct_data(i)>=0.5 && b_correct_data(i)<0.5
        if b_correct_data(i)<=a_correct_data(i)-0.5 && b_interval(i)>1
            b_interval(i)=b_interval(i)-1;
        end
    end

    if a_correct_data(i)<0.5 && e_correct_data(i)>=0.5
        if e_correct_data(i)>a_correct_data(i)+0.5 && e_interval(i)<10
            e_interval(i)=e_interval(i)+1;
        end
    end
    if a_correct_data(i)>=0.5 && e_correct_data(i)<0.5
        if e_correct_data(i)<=a_correct_data(i)-0.5 && e_interval(i)>1
            e_interval(i)=e_interval(i)-1;
        end
    end
