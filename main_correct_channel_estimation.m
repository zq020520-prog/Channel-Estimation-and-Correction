...
for i = 1:m
    ...
    g=A(2);
    h_abs=abs(h)^2;
    interval_true(i) =calculate_interval(h_abs); 

    % 噪声
    noise_std = sqrt(2 / (2 * SNR));
    ...
    a_interval(i)=calculate_interval(ha_abs_array);
    b_interval(i)=calculate_interval(hb_abs_array);
    e_interval(i)=calculate_interval(ge_abs_array);

    % 累积 MSE
    mse_a = mse_a + abs(ha_temp - h)^2/abs(h)^2;
    mse_b = mse_b + abs(hb_temp - h)^2/abs(h)^2;
    mse_ab =mse_ab + abs(ha_temp - hb_temp)^2/abs(hb_temp)^2;
         if a_interval(i)~=b_interval(i)
                 s=s+1;
         end
         if b_interval(i)~=interval_true(i)
                 j=j+1;
         end
...
for i=1:m
    ...
    a_small_inteval(i)=calculate_small_inteval(a_interval(i),ha_abs_array(i));
    b_small_inteval(i)=calculate_small_inteval(b_interval(i),hb_abs_array(i)); 
    e_small_inteval(i)=calculate_small_inteval(e_interval(i),ge_abs_array(i)); 
    ...
    if a_small_inteval(i)>3 && e_small_inteval(i)<a_small_inteval(i)-3 && e_interval(i)>1
             ...
             if a_interval(i)~=interval_true(i)
                 o=o+1;
             end
